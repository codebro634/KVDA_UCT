...
namespace OTH {
    ...
    struct Gamestate: public ABS::Gamestate{
        ...
        std::map<int,std::set<std::pair<int,int>>> legal_action_to_dirs = {};
        bool actions_calculated = false;
    ...
    class Model: public ABS::Model, public WLG::Model
    {
        ...
        private:
            static std::set<std::pair<int,int>> actionToLegalDirs(const Gamestate* state, int row, int col);
